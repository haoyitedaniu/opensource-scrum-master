#!/bin/bash
cd /root/opensource-scrum-master/server && (SCRUM_ADMIN_USERNAME="greatmaster" SCRUM_ADMIN_PASSWORD="password" npm run seed-admin-user 2>&1 > /root/scrum-seed.log.txt) &
